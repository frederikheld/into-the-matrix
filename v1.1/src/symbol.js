...
class Symbol extends HTMLElement {
  ...
  rigElement() {
    ...
    this.style.left =
      this.options.symbolSize * this.options.widthScalingFactor * this.column +
      "px";
    this.style.fontSize =
      this.options.symbolSize * this.options.fontSizeScalingFactor + "px";
    this.style.textShadow =
      "0 0 " +
      this.options.symbolSize / 4 +
      "px rgba(42, 255, 42), 0 0 " +
      this.options.symbolSize / 3 +
      "px rgba(42, 255, 42)";
    this.style.height = this.options.symbolSize + "px";
    this.style.width =
      this.options.symbolSize * this.options.widthScalingFactor + "px";
    this.style.lineHeight = this.options.symbolSize + "px";

    this.innerText =
      this.characters[Math.floor(Math.random() * (this.characters.length - 1))]; // duplicate with render()
  }
  ...
  async render() {
    return new Promise((resolve, reject) => {
      ...
      // this.style.filter = `opacity(${opacity})`;
      // @PERFORMANCE: `filter: opacity()` might use hardware acceleration, `opacity` won't
      // source: https://developer.mozilla.org/en-US/docs/Web/CSS/filter-function/opacity
