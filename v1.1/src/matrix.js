...
class Matrix extends HTMLElement {
    ...
    if (!this.availableRenderModes.includes(this.options.renderMode)) {
      console.warn(
        "`" +
          ...
          "` is not a valid value for `options.renderMode`! Defaulting to `" +
          this.availableRenderModes[0] +
          "`. Valid options are: " +
          this.availableRenderModes.map((el) => "`" + el + "`").join(", ") +
          "."
      );
  ...
  render(returnNewDOMElement = false) {
    ...
    for (let i = 0; i < this.children.length; i++) {
      ...
      if (this.children[i].children.length <= 0) {
        ...
        this.children[i].render(this.height);
      }
    }
  ...
  resize() {
    ...
    this.columns = Math.floor(
      parseInt(getComputedStyle(this.parentEl).width) /
        (this.options.symbolSize * this.options.widthScalingFactor)
    );
  ...
  run(cadence = 1000, statsEl = undefined) {
    if (this.options.renderMode === "set-interval") {
      ...
      this.animationHandler = requestAnimationFrame(() => {
        ...
        if (
          !this.lastRenderTime ||
          currentRenderTime > this.lastRenderTime + cadence
        ) {
          this.render();
          if (statsEl) {
            this.updateStats(statsEl);
  ...
  updateStats(statsEl) {
    statsEl.innerHTML = `# of cols: ${this.columns} | # of trickles: ${
      this.children.length
    } | render time: ${this.renderTime} ms | fps: ${Math.ceil(
      1000 / this.renderTime
    )}`;
  }
}
